...
class SMNet(nn.Module):
    ...
    def encode(self, features, proj_indices, masks_inliers):
        ...
        observed_masks = torch.zeros(
            (N, 250, 250), dtype=torch.bool, device=self.device
        )
        ...
        for t in range(T):
            ...
            if N > 1:
                ...
                batch_offset[1:] = torch.cumsum(
                    mask_inliers.sum(dim=1).sum(dim=1), dim=0
                )[:-1]
            ...
            if m.any():
                feature = F.interpolate(
                    feature, size=(480, 640), mode="bilinear", align_corners=True
                )
                if self.ego_downsample:
                    feature = feature[:, :, ::4, ::4]
                ...
                if self.mem_update == "lstm":
                    tmp_state = (
                        state[0][tmp_top_down_mask, :].to(self.device),
