...
obj_files = os.listdir(pc_dir)

houses_dim = json.load(open('data/houses_dim.json', 'r'))

output_dir = 'data/GT/semmap'
info = {}
...
for obj_f in tqdm(obj_files):
    ...
    f = h5py.File(os.path.join(pc_dir, obj_f), 'r')
    ...
    sizes += 2 # -- pad env bboxes -- not so accurate in the dataset.
...
json.dump(info, open(os.path.join('/'.join(output_dir.split('/')[:2]), 'semmap_GT_info.json'), 'w'))
